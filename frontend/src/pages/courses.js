...
const Page = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const courseList = response.data.courseList.filter((course) => course.instructor !== null);
  ...
  const saveCourses = async () => {
    try {
      ...
      const response = await axios.patch(
        `${configService.url}/users/courselist/${values.id}`,
        selectedCourses
      );
        ...
        <Container maxWidth="xl">
          <Stack spacing={3}>
            <Stack direction="row" justifyContent="space-between" spacing={4}>
              <Stack spacing={1}>
                <Typography variant="h4">Active Courses In {semesterData.description}</Typography>
              </Stack>
